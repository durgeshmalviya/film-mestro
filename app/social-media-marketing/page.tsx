...
async function getImageKitUrl(path: string): Promise<string> {
  ...
  // 3. Fetch signed URL from your API
  try {
...
// ============ CLEAN PATHS ONLY (no signed URLs) ============
const introImagePaths = [
  "/editorial/maestrofilms-12.jpg",
  "/editorial/maestrofilms-1.JPG",
];

const stripImagePaths = [
  "/editorial/MF_08998.jpg",
  "/editorial/MF_09874.jpg",
  "/editorial/Copy of 19.jpg",
  "/editorial/DSC05310.JPG",
];

export default function AdvancedWorkPage() {
  const [tokenMap, setTokenMap] = useState<Record<string, string>>({});
  const [introImages, setIntroImages] = useState<string[]>(introImagePaths);
  const [stripImages, setStripImages] = useState<string[]>(stripImagePaths);

  // Resolve ImageKit paths via API
  useEffect(() => {
    let cancelled = false;

    async function resolveImages() {
      const [resolvedIntro, resolvedStrip] = await Promise.all([
        Promise.all(introImagePaths.map((p) => getImageKitUrl(p))),
        Promise.all(stripImagePaths.map((p) => getImageKitUrl(p))),
      ]);

      if (!cancelled) {
        setIntroImages(resolvedIntro);
        setStripImages(resolvedStrip);
      }
    }
  ...
  useEffect(() => {
    const loadTokens = async () => {
      const ids = [HERO_PLAYBACK_ID, SOCIAL_PLAYBACK_ID];
      const tokens: Record<string, string> = {};

      await Promise.all(
        ids.map(async (id) => {
          tokens[id] = await getSignedPlaybackToken(id);
        })
      );
  ...
  const heroToken = tokenMap[HERO_PLAYBACK_ID];
  const socialToken = tokenMap[SOCIAL_PLAYBACK_ID];
  ...
  return (
    <>
      ...
      >
          ...
          <div className="relative z-10 max-w-[1400px] mx-auto w-full px-5 md:px-10 pb-16 md:pb-24 pt-32">
            <motion.div variants={fadeUp}>
              ...
              <div className="mb-8 space-y-1.5">
                <p className="text-[11px] md:text-[12px] tracking-[0.28em] uppercase text-[#c9a86c]/90 font-medium">
                  Exclusively for the Fashion Industry
                </p>
                <p className="text-[13px] md:text-sm tracking-[0.18em] uppercase text-[#e8e2d9]/85 font-light">
                  Fashion • Beauty • Jewellery
                </p>
                <p className="text-[12px] tracking-[0.35em] uppercase text-[#c9a86c] font-medium">
                  Nothing Else.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link
                  href="/contact"
                  className="inline-flex items-center justify-center gap-3 bg-[#c9a86c] text-transparent px-8 py-4 text-[12px] tracking-[0.2em] uppercase font-medium transition-all duration-500 hover:bg-[#e0c48a] hover:shadow-[0_0_40px_rgba(201,168,108,0.35)]"
                >
                  Book a Session
          ...
          <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center">
            <motion.div variants={fadeUp} className="lg:col-span-5">
              ...
              <div className="mb-6 space-y-1">
                <p className="text-[11px] tracking-[0.25em] uppercase text-[#c9a86c] font-medium">
                  Exclusively for the Fashion Industry
                </p>
                <p className="text-[12px] tracking-[0.18em] uppercase text-[#5a5a5a] font-light">
                  Fashion • Beauty • Jewellery — Nothing Else.
                </p>
              </div>
        ...
        {/* IMAGE STRIP */}
        <section className="py-5 md:py-6 px-5 md:px-10 bg-[#f0ebe4]">
        ...
        {/* CTA */}
        <section className="relative py-5 md:py-7 px-5 md:px-10 overflow-hidden">
          <div className="absolute inset-0">
            {heroToken ? (
              <MuxPlayer
                playbackId={HERO_PLAYBACK_ID}
          ...
          <div className="relative z-10 max-w-[1400px] mx-auto text-center">
            <motion.div variants={fadeUp}>
              ...
              <div className="mb-8 space-y-1.5">
                <p className="text-[11px] tracking-[0.28em] uppercase text-[#c9a86c]/90 font-medium">
                  Exclusively for the Fashion Industry
                </p>
                <p className="text-[13px] tracking-[0.18em] uppercase text-[#e8e2d9]/80 font-light">
                  Fashion • Beauty • Jewellery
                </p>
                <p className="text-[12px] tracking-[0.35em] uppercase text-[#c9a86c] font-medium">
                  Nothing Else.
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                <Link
                  href="/contact"
                  className="inline-flex items-center justify-center gap-3 bg-[#c9a86c] text-transparent px-10 py-4 text-[12px] tracking-[0.2em] uppercase font-medium transition-all duration-500 hover:bg-[#e0c48a] hover:shadow-[0_0_50px_rgba(201,168,108,0.35)]"
                >
                  Start a Project
